...
# create a data frame of lname, most popular year for that name, and count for that year
bn.topyear <- bn %>% group_by(lname, year) %>% summarise(count=sum(n)) %>% filter(row_number(desc(count))==1)

# create a data frame of lname, all-time count of F and M, and all-time total
bn.alltime <- bn %>% group_by(lname, sex) %>% summarise(count=sum(n)) %>% dcast(lname ~ sex, value.var="count") %>% tbl_df()
